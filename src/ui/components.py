...
def file_uploader() -> Optional[st.runtime.uploaded_file_manager.UploadedFile]:
    ...
    if uploaded_file:
        ...
        with st.expander("Preview file details"):
            ...
            st.write(
                f"**File size:** {round(len(uploaded_file.getvalue())/1024, 2)} KB"
            )
...
def tabs_interface(
    summary_callback: Callable, quiz_callback: Callable, has_pdf_content: bool
):
    ...
    with tab1:
        ...
        if has_pdf_content:
            if (
                st.button(" Generate Summary", key="gen_summary")
                or "summary_text" in st.session_state
            ):
                summary_callback()
        else:
    ...
    with tab2:
        ...
        if has_pdf_content:
            if (
                st.button(" Generate Quiz", key="gen_quiz")
                or "quiz_data" in st.session_state
            ):
                quiz_callback()
        ...
        with question_container:
            ...
            try:
                ...
                if (
                    ...
                    and st.session_state.user_answers[i] in q["options"]
                ):
            ...
            if st.session_state.quiz_submitted:
                ...
                if is_correct:
                    ...
                    st.error(
                        f"✗ Incorrect. The correct answer is: {q['correctAnswer']}"
                    )
...
def display_ollama_status(
    is_connected: bool, model_name: str, available_models: List[str] = None
):
    ...
    if is_connected:
        if model_name in (available_models or []):
            ...
            st.sidebar.warning(
                f"⚠️ Ollama is running, but model '{model_name}' is not available."
            )
